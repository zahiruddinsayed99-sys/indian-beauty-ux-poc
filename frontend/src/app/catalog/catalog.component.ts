...
@Component({
  ...
  template: `
    <div class="flex justify-between items-center mb-6">
      ...
      <select class="border border-gray-300 rounded p-2 focus:outline-none focus:border-pink-500">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      <div *ngFor="let p of products" class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-shadow flex flex-col">
        <div class="relative h-64 bg-gray-100 flex items-center justify-center">
          <img [src]="p.image" alt="product" class="object-cover w-full h-full opacity-50" />
        ...
        <div class="p-5 flex-1 flex flex-col">
          ...
          <div class="flex items-end gap-3 mt-auto mb-4">
            <span class="text-2xl font-extrabold text-gray-900">₹{{p.selling_price_inr}}</span>
          ...
          <div class="flex gap-2">
            <button (click)="addToCart(p)" class="flex-1 bg-pink-600 hover:bg-pink-700 text-white font-bold py-2 rounded shadow transition-colors">
              Add to Cart
            </button>
            <a [routerLink]="['/product', p.id]" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold py-2 rounded text-center shadow-sm transition-colors">
              Customize
            </a>
          </div>
        </div>
